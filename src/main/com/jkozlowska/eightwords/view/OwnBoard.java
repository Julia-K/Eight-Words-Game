...
public class OwnBoard extends Scene {
    ...
    private void createButtonsAndSetStyle() {
        ...
        for(int i = 0; i < 6; i++) {
            ...
            ((Button)buttons.getChildren().get(i)).setPrefSize(AllValues.buttonWidth,AllValues.buttonHeight);
        }
    ...
    private void update() {
        for (int i = 0; i < square.length; i++) {
            for (int j = 0; j < square[i].length; j++) {
                Rectangle rectangle = new Rectangle((AllValues.windowHeight-20-(10*gameBoard.getSize()+10))/gameBoard.getSize(), (AllValues.windowHeight-20-(10*gameBoard.getSize()+10))/gameBoard.getSize());
        ...
        for (int i = 0; i < gameBoard.getSize(); i++) {
            for (int j = 0; j < gameBoard.getSize(); j++) {
                ...
                if(gameBoard.isCellChangePossible(i,j)) {
                    ...
                    square[i][j].getChildren().forEach(item -> item.setOnMouseClicked(mouseEvent -> {
